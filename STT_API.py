...
# 텍스트 파일 및 오디오 파일 세팅
def get_text(file_name, data):
    if data=="저음질":
        with open(file_name, 'r') as file:
            answer_text = [line.strip() for line in file.readlines()][0]
    elif data=="eval_clean" or data=="eval_other":
...
def get_audioPath(file_name, data):
    if data=="저음질":
...
def etri_stt(loaded_list, data):
    ...
    for file_name in tqdm(loaded_list):
        ...
        answer_text=get_text(file_name, data)
        audioFilePath = get_audioPath(file_name, data)
...
# returnzero
def returnzero_stt(loaded_list, data):
...
# 네이버
def naver_stt(loaded_list, data):
